...
| [`.claude/hooks/session-start.sh`](.claude/hooks/session-start.sh) | SessionStart hook: install the dev/test toolchain so 'npm run health', smoke, render-check and the linters work in Claude Code on the web. |
