# Skills router (generated)

The routing map of skills by area. Imported into `CLAUDE.md` so the always-loaded
file stays lean. The block below is **auto-generated** from each skill's
frontmatter `area:` by `node tools/gen-docs.mjs` (and the PostToolUse `gen-docs`
hook) — don't edit between the markers; add/route a skill via the `skill-router` skill.
