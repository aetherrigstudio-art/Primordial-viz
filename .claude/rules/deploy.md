...
## ⚠️ Client-side privacy — NO AI/tooling fingerprints in the deployed site

The site is **no-build static**, so everything shipped (`index.html` + `src/`)
is human-readable via View Source. **Hard requirement: a visitor must find no
proof the site was AI-assisted and no reachable "AI" to probe.** When touching
anything that gets deployed:

- **No references to AI/dev tooling in `index.html` or `src/`** — no `claude` /
  `anthropic` / `mcp` / `tools/…` / `test/…` / "generated by", no commented build
  or generator commands. Keep code comments tool-agnostic.
- **No secrets/keys and no calls to any AI/LLM/MCP endpoint** from client code.
  The MCP server (`tools/mcp/`) is dev-only and must never be deployed or
  reachable from the site. The only `fetch()` is same-origin look JSON.
- **No debug globals / console noise** that expose internals or a test pipeline
  (gate or drop `window.__*`).
- **`.htaccess` must** disable directory listing (`Options -Indexes`) and deny
  dotfiles (`<FilesMatch "^\.">`) so the source tree can't be enumerated and the
  FTP-Deploy-Action's `.ftp-deploy-sync-state.json` manifest isn't served.
- The repo is **private**; git commits carry `Co-Authored-By: Claude` trailers —
  history is AI proof, so do **not** make the repo public without scrubbing it.
