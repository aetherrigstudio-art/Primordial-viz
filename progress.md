...
- [ ] **Wedding-planner landing page — VISION captured, discussion paused** | New client (a wedding planner). A two-act, fully-interactive scroll-through WebGL landing page: ACT 1 = a candlelit ivory-draped **pagoda hallway** that morphs continuously as you scroll (drapes thin → turn sheer → ivy thickens → replaced by ivy + flowers); scroll = travel, click = part a drape, audio = fabric *breathing* to lush cinematic strings (Piero-Piccioni vibe; reuses primordial's audio engine, retuned gentle). ACT 2 = an interactive flower garden (TBD). The planner's **words drift through the whole journey** (real text under the hood). Aesthetic locked by an operator-supplied reference (ivory drapes, floating candles, climbing greenery, dreamy — NOT grunge). Full capture: `clients/wedding-pagoda/VISION.md`. OPEN: act-2 interaction, the words' voice/content, where brand+contact lives, audio source, and scope (own repo vs here). Reuses the dynamic-canvas deep-research + reference-gather method. NEXT: resume discussion (act 2 or the words) OR brainstorm→spec→plan act 1. Do NOT build until "go". | parked 2026-06-20
- [x] **DONE 2026-06-20 — Buff the RAG with landing-page craft context** | Completed: added 6 our-own-words craft notes + a dynamic-canvas deep-research report under `research/landing-page-rag/`, in the RAG index with probes; plus frontpage reel-study + reverse-engineering-targets + volumetric/dynamic-canvas notes. (Superseded/extended by the wedding-pagoda thread above.)
