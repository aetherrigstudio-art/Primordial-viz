...
- [ ] **Front-page visual — WORKSHOP IT (next session, after /clear)** | Design the real signature visual for `/Test/`, replacing the placeholder slime. Use the `visual-workshop` skill + `workshop/` rig. Full launch brief (charter, constraints, first batch of 3 sketches to render & react — `fp-slime` / `fp-hybrid` / `fp-geo`, open questions on palette/how-geometric): `workshop/sketches/frontpage/BRIEF.md`. Operator chose to workshop (react to clips) rather than pick a motif from text. | set up 2026-06-20
